...
*   **琴鍵排列**：
    ...
    *   **佈局優化**：底部控制列需依 viewport 大小自動預留底部空間，避免被手機瀏覽器底部工具列/網址列遮擋。
*   **音名標示**：
    *   採用簡譜 (Jianpu) 系統。
...
*   **控制面板**：
    *   新增最低音/最高音動態範圍控制。
    *   最低音下限為 ˙1，最高音上限為 7˙˙。
    *   預設範圍為 1～3˙˙，且最高音設定值必須大於最低音。
    *   新增 Release 控制：1.0～4.0 秒、每 0.5 秒一級、預設 2.5 秒。
    *   新增音量平衡控制：1x～10x、預設 5x。
    *   音量平衡用於弭平高音聽感較大聲的差異，並可提升最低音音量；高倍率時需搭配全頻段增益優化避免失真。
...
*   **包絡控制 (ADSR Envelope)**：
    ...
    *   Release: 可調整為 1.0～4.0 秒（每 0.5 秒），預設 2.5 秒，並於設定時間內指數衰減至靜音。
*   **音孔遮擋 (Wah-wah) 效果**：
    *   按住音孔時啟用遮擋效果，放開後恢復原始音色。
    *   遮擋與恢復皆採平滑過渡，避免音色突變。
*   **音頻上下文**：包含使用者互動後的 `ctx.resume()` 機制，以符合瀏覽器自動播放策略。

### 4.3 音階配置 (Key Configuration)
完整 17 音配置固定如下；實際可彈奏範圍由最低音/最高音控制動態限制。
從左至右排列如下：
1.  **D6** (2˙˙) - 1174.66 Hz
2.  **B5** (7˙) - 987.77 Hz
3.  **G5** (5˙) - 783.99 Hz
